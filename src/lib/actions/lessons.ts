...
export async function submitStepAnswer(
  ...
  const guestId = userData.user ? null : await ensureGuestId();
...
export async function completeLesson(
  ...
  // Mastery from verse-linked steps in this lesson.
  ...
  // Unlock next units that require this unit.
  const { data: nextUnits } = await supabase
    .from("learning_units")
    .select("id")
    .eq("unlock_after_unit_id", lesson.unit_id);
  if (nextUnits && nextUnits.length > 0) {
    // Unlock when all lessons in prerequisite unit are done — simplified: unlock on any lesson complete in unit.
...
export async function startLesson(
  ...
  if ("error" in auth) {
    // Guests can start guest lessons without a progress row.
    return { ok: true };
  }
