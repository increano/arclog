...
/** OT vs NT for library grouping (deuterocanonical treated as OT). */
export function testamentForBookCode(code: string): "OT" | "NT" {
  const logical = LOGICAL_BY_CODE.get(code.trim().toUpperCase().replace(/\./g, ""));
  if (logical && NT_LOGICAL.has(logical)) return "NT";
  return "OT";
}
