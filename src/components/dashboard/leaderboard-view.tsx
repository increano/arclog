...
export function LeaderboardView({
  ...
  const goalXp = Math.max(500, memberCount * 100);
  const challengeProgress = Math.min(
    100,
    Math.round((totalWeeklyXp / goalXp) * 100)
  );
        ...
        <div className="flex flex-col gap-6 md:flex-row md:items-center">
          ...
          <div className="flex-1">
            ...
            <p className="mb-4 font-medium text-on-surface-variant">
              ...
              {totalWeeklyXp.toLocaleString()} XP earned this week.
            </p>
            <ProgressBar
              value={challengeProgress}
              label="Group Progress"
              detail={`${challengeProgress}% of ${goalXp.toLocaleString()} XP`}
            />
          </div>
        </div>
      </DashboardCard>
      ...
      {entries.length === 0 ? (
        <DashboardCard>
          <p className="font-medium text-on-surface-variant">
            No league members yet. Complete a lesson to start earning weekly XP.
          </p>
        </DashboardCard>
      ) : (
        <>
              ...
              return (
                <DashboardCard
                  ...
                  className={`flex flex-col items-center text-center ${
                    isFirst
                      ? "bg-primary-container text-on-primary-container md:scale-105"
                      : ""
                ...
                >
                  ...
                  <h3
                    className={`text-lg font-bold ${isFirst ? "text-white" : "text-on-surface"}`}
                  >
